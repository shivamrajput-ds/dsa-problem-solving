**# DSA & Competitive Programming Journey**

A structured ****24-week journey**** focused on improving:

* Data Structures & Algorithms

* Competitive Programming

* Problem Solving

* Pattern Recognition

* Software Engineering Interview Preparation

* Long-term algorithmic thinking

The goal of this repository is ****not to collect hundreds of Accepted submissions****.

The goal is to improve the complete problem-solving process:

*> Understand constraints → find observations → choose the right approach → implement cleanly → analyze complexity → learn from mistakes → revisit weak problems.*

---

**## Profiles**

* ****Codeforces:**** [Shivam_rajp](https://codeforces.com/profile/Shivam_rajp)

* ****LeetCode:**** [ShivamSynapse](https://leetcode.com/u/ShivamSynapse/)

---

**## The Plan**

I am following a structured ****24-week DSA + Competitive Programming roadmap**** containing approximately ****230–250 carefully selected problems****.

The focus is on ****quality over quantity****.

I do not randomly solve hundreds of similar questions or continuously switch between different sheets.

---

**## Weekly System**

**### Monday – Friday**

* 2 quality problems per day

* Short concept study when required

* Independent problem solving

* Dry runs and edge-case analysis

* Time and space complexity analysis

* Mistake tracking

**### Saturday**

* Codeforces / AtCoder contest or virtual contest

* Timed problem solving

* Focus on speed, accuracy, and independent thinking

**### Sunday**

* Contest upsolving

* Re-solving previous mistakes

* Weekly revision

* Weak-area analysis

---

**# 24-Week Roadmap**

**## Phase 1 — Problem-Solving Foundations**

* Complexity Analysis

* Sorting

* Hashing

* Prefix Sum

* Difference Array

* Two Pointers

* Sliding Window

**## Phase 2 — Core DSA Patterns**

* Binary Search

* Binary Search on Answer

* Greedy

* Intervals

* Stack

* Monotonic Stack

* Heap / Priority Queue

* Recursion

* Backtracking

* Bit Manipulation

**## Phase 3 — Trees & Graphs**

* Binary Trees

* Tree DFS / BFS

* BST

* Tree Diameter

* Subtree Problems

* Graph DFS / BFS

* Connected Components

* Cycle Detection

* Bipartite Graphs

* Topological Sorting

* Disjoint Set Union

* Dijkstra

* Minimum Spanning Tree

* Shortest Paths

**## Phase 4 — Dynamic Programming**

* DP Fundamentals

* 1D DP

* Take / Not Take

* Coin Change

* Knapsack

* Grid DP

* LIS

* LCS

* String DP

* DAG DP

* Tree DP

* Interval DP

**## Phase 5 — Competitive Programming Techniques**

* Number Theory

* GCD / LCM

* Prime Numbers

* Sieve

* Modular Arithmetic

* Fast Exponentiation

* Combinatorics

* Bitmasking

* Fenwick Tree

* Segment Tree

* Trie

* Binary Lifting

**## Phase 6 — Mixed Problem Solving**

* Mixed Codeforces Problems

* Interview-style DSA

* Timed Problem Solving

* Contests

* Upsolving

* Weak-area Repair

* Revision

---

**# Problem Sources**

I intentionally use a small number of high-quality platforms:

* ****Codeforces**** — Competitive programming and problem-solving

* ****CSES**** — Algorithm fundamentals

* ****LeetCode**** — Interview-oriented DSA

* ****AtCoder**** — Algorithmic thinking and Dynamic Programming

---

**# Repository Structure**

```text

dsa-cp-practice/

│

├── README.md

│

├── week-01/

│   ├── day-01/

│   │   ├── beautiful_matrix.py

│   │   ├── theatre_square.py

│   │   └── README.md

│   │

│   ├── day-02/

│   │   ├── helpful_maths.py

│   │   ├── queue_at_the_school.py

│   │   └── README.md

│   │

│   ├── day-03/

│   │   ├── dubstep.py

│   │   ├── amusing_joke.py

│   │   └── README.md

│   │

│   ├── day-04/

│   │   ├── distinct_numbers.py

│   │   ├── maximum_subarray_sum.py

│   │   └── README.md

│   │

│   └── day-05/

│       ├── best_time_to_buy_and_sell_stock.py

│       ├── valid_anagram.py

│       └── README.md

│

├── week-02/

│

├── contests/

│   ├── codeforces/

│   └── atcoder/

│

└── revision/

```

---

**# Current Progress**

****Current Week:**** Week 01

****Current Focus:**** Implementation, Complexity, Linear Scans & Frequency Counting

****Problems Completed:**** 10

| Day    | Problems                                       | Main Skill                       | Status |

| ------ | ---------------------------------------------- | -------------------------------- | ------ |

| Day 01 | Beautiful Matrix, Theatre Square               | Implementation + Math            | ✅      |

| Day 02 | Helpful Maths, Queue at the School             | Frequency + Simulation           | ✅      |

| Day 03 | Dubstep, Amusing Joke                          | Strings + Frequency Counting     | ✅      |

| Day 04 | Distinct Numbers, Maximum Subarray Sum         | Sorting + Kadane's Algorithm     | ✅      |

| Day 05 | Best Time to Buy and Sell Stock, Valid Anagram | Linear Scan + Frequency Counting | ✅      |

---

**# Weekly Progress**

| Week    | Main Focus                           | Status         |

| ------- | ------------------------------------ | -------------- |

| Week 01 | Implementation, Sorting & Complexity | 🟡 In Progress |

| Week 02 | Prefix Sum & Difference Array        | ⬜              |

| Week 03 | Two Pointers & Sliding Window        | ⬜              |

| Week 04 | Binary Search                        | ⬜              |

| Week 05 | Greedy & Intervals                   | ⬜              |

| Week 06 | Stack, Heap & Backtracking           | ⬜              |

| Week 07 | Binary Trees                         | ⬜              |

| Week 08 | Tree Algorithms                      | ⬜              |

| Week 09 | Graph Traversal                      | ⬜              |

| Week 10 | DSU & Shortest Paths                 | ⬜              |

| Week 11 | Advanced Graph Foundations           | ⬜              |

| Week 12 | Bit Manipulation & Backtracking      | ⬜              |

| Week 13 | DP Fundamentals                      | ⬜              |

| Week 14 | Knapsack & Coin DP                   | ⬜              |

| Week 15 | Sequence & String DP                 | ⬜              |

| Week 16 | Grid, Tree & DAG DP                  | ⬜              |

| Week 17 | Number Theory                        | ⬜              |

| Week 18 | Bitmasking & Combinatorics           | ⬜              |

| Week 19 | Fenwick Tree                         | ⬜              |

| Week 20 | Segment Tree                         | ⬜              |

| Week 21 | Trie & Binary Lifting                | ⬜              |

| Week 22 | Advanced DP / Tree Integration       | ⬜              |

| Week 23 | Interview DSA                        | ⬜              |

| Week 24 | Mixed Problems & Weak-Area Repair    | ⬜              |

---

**# Problem-Solving Process**

For every problem:

```text

Understand the problem

        ↓

Read constraints

        ↓

Think of brute force

        ↓

Identify repeated work

        ↓

Find observations

        ↓

Choose algorithm / data structure

        ↓

Dry run

        ↓

Implement

        ↓

Test edge cases

        ↓

Analyze time & space complexity

        ↓

Record mistakes

```

---

**# Getting-Stuck Rule**

**## First 15 Minutes**

* Understand examples

* Inspect constraints

* Think of brute force

* Write observations

* Try small test cases manually

**## Around 30 Minutes**

If completely stuck:

* Take a small hint

* Do not immediately read the complete solution

**## Around 45–60 Minutes**

If the problem is near my current level and I still cannot derive the solution:

* Study the core observation

* Close the editorial

* Implement the solution independently

* Add the problem to the revision queue

---

**# Revision System**

Problems where I required an important hint or editorial are revisited approximately:

```text

Initial Solve

     ↓

After 3 Days

     ↓

After 7 Days

     ↓

After 30 Days

```

The objective is not to remember the code.

The objective is to ****reconstruct the idea independently****.

---

**# Mistake Categories**

| Code  | Meaning                    |

| ----- | -------------------------- |

| PR    | Pattern Recognition        |

| OBS   | Wrong / Missed Observation |

| EDGE  | Edge Case                  |

| IMP   | Implementation Bug         |

| CMP   | Complexity Mistake         |

| MATH  | Mathematical Mistake       |

| DS    | Wrong Data Structure       |

| PANIC | Contest / Time Management  |

---

**# What I Track**

I care more about improvement than raw problem count.

Important metrics:

* Problems solved independently

* Time taken to find the approach

* Problems solved without hints

* Contest performance

* Problems successfully upsolved

* Previously missed problems solved independently later

* Patterns recognized correctly

* Implementation mistakes

* Complexity-analysis accuracy

* Progress in problem difficulty

---

**# Current Learnings**

**## Day 01**

**### Beautiful Matrix**

* 2D matrix traversal

* Finding an element efficiently

* Row and column distance

* `abs()` usage

* Breaking nested search after finding the answer

**### Theatre Square**

* Converting a problem statement into mathematics

* Ceiling division

* Avoiding unnecessary simulation

---

**## Day 02**

**### Helpful Maths**

* Frequency counting

* Sorting-like behavior without necessarily using comparison sorting

* Understanding that consecutive loops can still combine to `O(n)`

**### Queue at the School**

* Simulation

* Careful state updates

* Correct pointer/index movement

* Understanding why `i += 2` is necessary after swapping `BG → GB`

* Deriving `O(t × n)` simulation complexity

---

**## Day 03**

**### Dubstep**

* String splitting using a custom separator

* Handling empty strings produced by `split()`

* Efficient string construction using `join()`

* Understanding why repeated string concatenation can be inefficient

**### Amusing Joke**

* Character frequency counting using `defaultdict`

* Exact frequency matching

* Understanding fixed-alphabet space complexity

* Recognizing that all final frequencies must become exactly `0`

---

**## Day 04**

**### Distinct Numbers**

* Sorting an array to group equal values together

* Counting distinct values using a single scan after sorting

* Comparing adjacent / previously seen values

* Understanding why sorting gives `O(n log n)` time complexity

* Learning that hash-based `O(n)` performance is average-case, while sorting can provide more predictable performance on adversarial judge inputs

* Using `split()` instead of `split(" ")` for cleaner competitive-programming input parsing

**### Maximum Subarray Sum**

* Kadane's Algorithm

* Maintaining a running subarray sum

* Updating the best answer during a single traversal

* Discarding a negative running prefix because it cannot improve a future subarray

* Correctly handling all-negative arrays by initializing the answer with `-inf`

* Deriving `O(n)` time and `O(1)` auxiliary space

---

**## Day 05**

**### Best Time to Buy and Sell Stock**

* Maintaining the minimum price seen so far

* Using a single left-to-right traversal

* Calculating the best possible profit for every selling day

* Avoiding the brute-force comparison of every buy/sell pair

* Understanding the observation:

```text

Current Profit = Current Price - Minimum Price Seen So Far

```

* Improving a potential `O(n²)` approach to `O(n)`

* Using clear variable names such as `min_price` and `max_profit`

* Deriving `O(n)` time and `O(1)` auxiliary space

**### Valid Anagram**

Solved using ****two approaches****.

**#### Approach 1 — Fixed Frequency Array**

* Using an array of size `26` for lowercase English letters

* Mapping characters using `ord(ch) - ord('a')`

* Incrementing frequencies for `s`

* Decrementing frequencies for `t`

* Verifying that every final frequency is `0`

* Understanding that fixed `26` storage gives `O(1)` auxiliary space

**#### Approach 2 — Hash Map**

* Using `defaultdict(int)` for frequency counting

* Checking string lengths before processing

* Decreasing frequencies while traversing `t`

* Returning early when a frequency becomes negative

* Understanding that hash-map space depends on the number of unique characters

* Learning why a dictionary-based approach adapts more naturally to Unicode characters

* Deriving `O(n + m)` time and `O(k)` space, where `k` is the number of unique characters

---

**# Long-Term Goal**

This 24-week roadmap is only the ****foundation****.

My long-term goal is to become exceptionally strong at:

* Competitive Programming

* Data Structures & Algorithms

* Mathematical Thinking

* Problem Solving

* Software Engineering Interviews

I understand that reaching a very high level in competitive programming requires ****years of deliberate practice****.

Therefore, I am not trying to rush.

I am focusing on:

*>* ****Consistency + Depth + Contests + Upsolving + Revision****

One problem at a time.

One pattern at a time.

One day at a time.
